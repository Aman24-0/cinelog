...
export function useEpisodeTracking({ movie, details, isPreview, isGuest, uid, activeServer, inferDurationSeconds, setForm, setWatchProgress, setPlayerStartProgress, showToast, onLogin, currentSeasonNumber, currentEpisodeNumber, isCompleted }) {
  ...
  const loadWatchedEpisodes = async () => {
    // ✅ FIX: isPreview() check hataya — preview mode mein bhi episodes load ho sakein
    // Lekin guest ya uid nahi ho toh skip karo
    if (isGuest || !uid || movie()?.media_type !== 'tv') return;
    try {
  ...
  return {
    selectedSeason, setSelectedSeason, seasonEpisodes, seasonsLoading,
    expandedEpisodes, setExpandedEpisodes, watchedEpisodes,
    tvSeasons, selectedSeasonData, selectedSeasonEpisodes,
    episodeDocId, getEpisodesForSeason,
    loadWatchedEpisodes, fetchSeasonEpisodes,
    toggleEpisodeWatched, checkIfWatched
  };
}
