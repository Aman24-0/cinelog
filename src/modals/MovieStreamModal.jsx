...
export const MovieStreamModal = (props) => {
  ...
  const handleWatchMovie = async (movie) => {
    ...
    try {
      // ✅ UPDATED: Use IMDB ID for Torrentio instead of Title/Year
      // Ensure imdb_id exists and starts with 'tt'
      const imdbId = movie.imdb_id         ? (movie.imdb_id.startsWith('tt') ? movie.imdb_id : `tt${movie.imdb_id}`)
        : null;

      if (!imdbId) {
        setSearchError('IMDB ID not found for this movie. Cannot fetch streams.');
        return;
      }
  ...
  const handlePlayStream = (stream) => {
    props.onVideoFound({
      ...
      videoUrl: stream.magnet, // ✅ UPDATED: Changed from stream.link to stream.magnet
      poster: selectedMovie().poster,
      source: 'torrentio', // ✅ UPDATED: Source name changed
    });
    setStreamList([]);
    ...
    <Show when={props.isOpen}>
      ...
      <div class="fixed inset-0 z-[100000] flex items-center justify-center p-4 pointer-events-none" onClick={props.onClose}>
        <div 
          ...
          style={{ 
            background: 'linear-gradient(135deg, rgba(14,16,24,0.95) 0%, rgba(8,9,11,0.95) 100%)',             border: '1px solid var(--border-active)', 
          ...
          <div class="sticky top-0 z-10 p-6 border-b shrink-0" style={{ borderColor: 'var(--border)' }}>
            <div class="flex items-center justify-between mb-4">
              <h2 class="font-headline text-3xl text-white flex items-center gap-3">
                <Icon name="movie" class="text-2xl" style="color: var(--p)" /> Stream Movies
              </h2>
              ...
              <div class="animate-fade-in">
                <div class="flex items-center gap-4 mb-6 pb-4 border-b border-white/10">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">{selectedMovie()?.title}</p>                  </div>
                </div>
