...
const scrapeVideoSource = publicProcedure
  ...
  .mutation(async ({ input }) => {    try {
      ...
      if (!response.ok) throw new Error('Here is the complete, updated `server/routers.js` file. This version completely removes Prowlarr, switches to Torrentio, and **automatically fetches the IMDB ID from TMDB** when you click watch so it works perfectly with your current search results.

### ✅ Updated `server/routers.js`
...
const scrapeVideoSource = publicProcedure
  ...
  .mutation(async ({ input }) => {
    try {
      ...
      // 2. Check cache first to avoid rate limits      const cacheKey = `${input.type}:${imdbId}`;
      const cached = torrentCache.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
        console.log(`📦 Cache hit for ${cacheKey}`);
        return cached.data;
...
const getTmdbConfig = publicProcedure.query(async () => {
  try {
    ...
    const config = await res.json();    return { success: true, imageBaseUrl: config.images.base_url };
  } catch (e) {
